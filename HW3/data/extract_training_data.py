...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        single_vector = np.zeros(6)
        for ran in range(1, min(len(state.stack),3)+1):
            if not words[state.stack[-ran]]: 
                single_vector[ran-1] = 3
            elif words[state.stack[-ran]].lower() in self.word_vocab:
                single_vector[ran-1] = self.word_vocab[words[state.stack[-ran]].lower()]
            elif pos[state.stack[-ran]] == "CD":
                single_vector[ran-1] = 0
            elif pos[state.stack[-ran]] == "NNP":
                single_vector[ran-1] = 1
            else:
                single_vector[ran-1] = 2
        for ran in range(1, min(len(state.buffer),3)+1):
            if not words[state.buffer[-ran]]: 
                single_vector[ran+2] = 3
            elif words[state.buffer[-ran]].lower() in self.word_vocab:
                single_vector[ran+2] = self.word_vocab[words[state.buffer[-ran]].lower()]
            elif pos[state.buffer[-ran]] == "CD":
                single_vector[ran+2] = 0
            elif pos[state.buffer[-ran]] == "NNP":
                single_vector[ran+2] = 1
            else:
                single_vector[ran+2] = 2
        return single_vector

    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        return keras.utils.to_categorical(self.output_labels[output_pair], one_hot_vector = 91)
